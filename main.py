...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    #newcode
    arr = np.linspace(start = 0.5, stop = 1, num = n)
    weights_list = []
    costs_list = []
    nr_expanded_states_list = []
    for weight in arr:
        a_star14 = AStar(heuristic_type, weight, max_nr_states_to_expand)
        res14 = a_star14.solve_problem(problem)
        if res14.is_solution_found:
            weights_list.append(weight)
            costs_list.append(res14.solution_g_cost)
            nr_expanded_states_list.append(res14.nr_expanded_states)
    plot_distance_and_expanded_wrt_weight_figure("A*",weights_list,costs_list,nr_expanded_states_list)
...
def toy_map_problem_experiments():
    ...
    res2 = a_star.solve_problem(toy_map_problem)
    print(res2)
    ...
    #newcode
    a_star2 = AStar(AirDistHeuristic)
    res3 = a_star2.solve_problem(toy_map_problem)
    print(res3)
...
def basic_deliveries_truck_problem_experiments():
    ...
    #newcode
    uc16 = UniformCost()
    res16 = uc16.solve_problem(small_delivery_problem_with_distance_cost)
    print(res16)
...
def deliveries_truck_problem_anytime_astar_experiments():
    ...
    #newcode
    anytime_star = AnytimeAStar(TruckDeliveriesMSTAirDistHeuristic, max_nr_states_to_expand_per_iteration = 50)
    res = anytime_star.solve_problem(moderate_delivery_problem_with_distance_cost)
    print(res)
...
def big_deliveries_truck_problem_with_non_acceptable_heuristic_and_anytime_astar_experiments():
    ...
    #newcode
    anytime_star = AnytimeAStar(TruckDeliveriesSumAirDistHeuristic, max_nr_states_to_expand_per_iteration=400)
    res = anytime_star.solve_problem(big_delivery_problem_with_distance_cost)
    print(res)

    anytime_star = AnytimeAStar(TruckDeliveriesMSTAirDistHeuristic, max_nr_states_to_expand_per_iteration=400)
    res = anytime_star.solve_problem(big_delivery_problem_with_distance_cost)
    print(res)
