...
class MapProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        #new code:
        for target in junction.outgoing_links:
            if self.road_cost_fn is None:
                operator_cost=target.distance
            else:
                operator_cost=self.road_cost_fn(target)
            successor_state = MapState(target.target)
            yield OperatorResult(successor_state, operator_cost)
