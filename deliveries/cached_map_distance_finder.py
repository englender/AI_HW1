...
class CachedMapDistanceFinder:
    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        if self._is_in_cache(key):
            ...
            map_problem = MapProblem(self.streets_map, src_junction.index, tgt_junction.index, self.road_cost_fn, self.zero_road_cost)
            sol = self.map_problem_solver.solve_problem(map_problem)
            if sol.is_solution_found:
                self._insert_to_cache(key, sol.solution_cost)
