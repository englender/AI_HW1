...
class DeliveriesTruckState(GraphProblemState):
    ...
    def __eq__(self, other):
        ...
        # newcode
        return (self.current_location == other.current_location) and (self.loaded_deliveries == other.loaded_deliveries) and (self.dropped_deliveries == other.dropped_deliveries)
...
class DeliveriesTruckProblem(GraphProblem):
    ...
    def _calc_map_road_cost(self, link: Link) -> DeliveryCost:
        ...
        if link.is_toll_road:
            toll_fee = self.problem_input.toll_road_cost_per_meter*link.distance
        else:
            toll_fee = 0
        return DeliveryCost(
            ...
            money_cost=gas_cost_per_meter*link.distance + toll_fee,
    ...
    def get_cost_lower_bound_from_distance_lower_bound(self, total_distance_lower_bound: float) -> float:
        ...
        if self.optimization_objective == OptimizationObjective.Distance:
            ...
            tmp_l = Link(
                source = -1,
                target = -1,
                distance = total_distance_lower_bound,
                highway_type = 1,
                max_speed = MAX_ROAD_SPEED,
                is_toll_road = False
            )
            return self._calc_map_road_cost(tmp_l).money_cost
